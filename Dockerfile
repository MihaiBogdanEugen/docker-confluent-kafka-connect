...
RUN echo "===> Installing Schema Registry (for Avro jars) ..." \
    ...
    && wget "https://raw.githubusercontent.com/confluentinc/cp-docker-images/3.3.x-quickstart-fix/debian/kafka-connect-base/include/etc/confluent/docker/apply-mesos-overrides" -O "/etc/confluent/docker/apply-mesos-overrides" \
    ...
    && wget "https://raw.githubusercontent.com/confluentinc/cp-docker-images/3.3.x-quickstart-fix/debian/kafka-connect-base/include/etc/confluent/docker/configure" -O "/etc/confluent/docker/configure" \
    && echo "3316f81e68b0437700c0f48170a6e13511bd103fa7d10771151370f9ff7f7064" "/etc/confluent/docker/configure" | sha256sum -c - \
    && wget "https://raw.githubusercontent.com/confluentinc/cp-docker-images/3.3.x-quickstart-fix/debian/kafka-connect-base/include/etc/confluent/docker/ensure" -O "/etc/confluent/docker/ensure" \
    && echo "d7ca27dbd0ada8b4fc5a016ee2bd2eb53b6d24f14cb2b4cee00e834ccc06bc74" "/etc/confluent/docker/ensure" | sha256sum -c - \
    && wget "https://raw.githubusercontent.com/confluentinc/cp-docker-images/3.3.x-quickstart-fix/debian/kafka-connect-base/include/etc/confluent/docker/launch" -O "/etc/confluent/docker/launch" \
